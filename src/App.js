...
class App extends Component {
  ...
  handlePutBlog = (blog, id) => {
    ...
    axios.put(`http://localhost:3000/blogs/${id}`, blog).then(res =>
      this.setState({
        blogs: [
          ...this.state.blogs.splice(
            this.state.blogs.indexOf(
              this.state.blogs.find(blog => blog._id === id)
            ),
            1,
            res.data
          )
      ...
      <Router>
        <div className='App'>
          ...
          <Route exact path='/blogs/api/post' render={props => (
            <Fragment>
              <PostBlog {...props} materias={materias} handlePostBlog={blog => this.handlePostBlog(blog)} />
            </Fragment>
          )}
          />
          ...
          <Route exact path='/blogs/api/put/:id' render={props => (
            <Fragment>
              <PutBlog {...props} blogs={blogs} materias={materias} handlePutBlog={(blog, id) => this.handlePutBlog(blog, id)} />
            </Fragment>
          )}
          />
